...
目前專案理解（2026-04-30）：
1. dataset 主來源為 Minari 格式資料集；專案多數流程以 `dataset_id` 配合 `minari.load_dataset(...)` 讀取資料，路徑根目錄由 `source/ice_offline/paths.py` 的 `minari_root()` 管理（預設 `tmps/datasets`）。
2. state 資訊有兩條主路：
   - online：透過 `StateCollector`（`source/ice_offline/dataset/state_collector.py`）在 `reset/step` 蒐集狀態，輸出 `state_data.hdf5`。
   - offline：透過 `convert_fullobs`（`source/ice_offline/dataset/state_convertor.py`）把 Minari episode 轉成 `State`，並寫入 `infos["state"]` 建立轉換後資料集。
3. 重播（replay）也有兩條主路：
   - online inject：`StateInjectWrapper`（`source/ice_offline/env/replay/state_inject_wrapper.py`）依資料集中的 state/obs/action/reward 做 transition 重播。
   - offline loader：`StateLoader`（`source/ice_offline/dataset/state_loader.py`）從 `state_data.hdf5` 讀取 episode 與 step。
4. value 流程目前以 state-action value（Q-like）為核心：
   - 蒐集：`ValueCollector`（`source/ice_offline/dataset/value_collector.py`）輸出 `value_data.hdf5` 的 `episode_x/values`。
   - 載入：`ValueLoader`（`source/ice_offline/dataset/value_loader.py`）讀取 `values` 供後續渲染與檢查。
