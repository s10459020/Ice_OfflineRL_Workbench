...
from generate_result_quick_tables import BASE_MEDIUM_TABLES
...
BASE_VERSION_PATH = RESULT_ROOT / "agent_dataset_versions.csv"
OUTPUT_PATH = RESULT_ROOT / "base_medium.csv"


def base_version_records() -> dict[tuple[str, str], dict[str, str]]:
    with BASE_VERSION_PATH.open("r", encoding="utf-8", newline="") as file:
        return {
...
def generate() -> Path:
    records = base_version_records()
    rows: list[list[str]] = []

    for dataset in BASE_MEDIUM_TABLES:
        lower = mean(dataset_returns(dataset.lower_id))
        upper = mean(dataset_returns(dataset.upper_id))
