...
# Mapping
# ====================
def _all_pairs(our: CQLAgent, ref):
...
def our_loss_actor(our: CQLAgent, batch) -> torch.Tensor:
    o, _, _, _, _ = batch
    a, log_prob = our.actor.sample(o)
    return our.loss_actor(batch, a, log_prob)

def our_loss_critic(our: CQLAgent, batch) -> torch.Tensor:
...
# Compare
# ====================
def build_our() -> CQLAgent:
    return CQLAgent(obs_size=OBS_DIM, act_size=ACT_DIM)
...
def init_compare() -> tuple[CQLAgent, object]:
    print_stage("Init")
    our = build_our()
    ref = build_ref()
    with torch.no_grad():
...
def compare_act(our: CQLAgent, ref) -> None:
...
def compare_loss(our: CQLAgent, ref) -> None:
...
def compare_param(our: CQLAgent, ref) -> None:
