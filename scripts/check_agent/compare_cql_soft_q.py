...
# Mapping
# ====================
def _all_pairs(our: CQLSoftQAgent, ref):
...
def ref_update_and_collect_params(ref, batch: TorchMiniBatch, step: int, our: CQLSoftQAgent):
...
def our_loss_actor(our: CQLSoftQAgent, batch) -> torch.Tensor:
    o, _, _, _, _ = batch
    a, log_prob = our.actor.sample(o)
    return our.loss_actor(batch, a, log_prob)

def our_loss_critic(our: CQLSoftQAgent, batch) -> torch.Tensor:
...
# Compare
# ====================
def build_our() -> CQLSoftQAgent:
    return CQLSoftQAgent(obs_size=OBS_DIM, act_size=ACT_DIM)
...
def init_compare() -> tuple[CQLSoftQAgent, object]:
    print_stage("Init")
    our = build_our()
    ref = build_ref()
    with torch.no_grad():
...
def compare_act(our: CQLSoftQAgent, ref) -> None:
...
def compare_loss(our: CQLSoftQAgent, ref) -> None:
...
def compare_param(our: CQLSoftQAgent, ref) -> None:
