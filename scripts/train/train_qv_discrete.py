...
RUNNER_ID = "qv_discrete_onerooms8"
BATCH_SIZE = 64
...
def eval_loss(agent: QVAgentDiscrete, episode_batch: tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]) -> dict[str, float]:
    o, a, r, on, d = episode_batch
    return {"loss": float(agent.loss_critic(o, a, r, on, d).item())}


def eval_loss_q(agent: QVAgentDiscrete, episode_batch: tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]) -> dict[str, float]:
    o, a, r, on, d = episode_batch
    return {"loss_q": float(agent._loss_q(o, a, r, on, d).item())}


def eval_loss_v(agent: QVAgentDiscrete, episode_batch: tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]) -> dict[str, float]:
...
def main() -> None:
    ...
    agent = QVAgentDiscrete(obs_size=obs_dim, act_size=act_size)
