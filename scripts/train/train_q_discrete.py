...
RUNNER_ID = "q_discrete_onerooms8"
BATCH_SIZE = 64
...
def eval_loss(agent: QAgentDiscrete, episode_batch: tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]) -> dict[str, float]:
    o, a, r, on, d = episode_batch
    return {"loss": float(agent.loss_critic(o, a, r, on, d).item())}


def eval_loss_q(agent: QAgentDiscrete, episode_batch: tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]) -> dict[str, float]:
    o, a, r, on, d = episode_batch
...
def main() -> None:
    ...
    agent = QAgentDiscrete(obs_size=obs_dim, act_size=act_size)
    ...
    runner.train(
        agent=agent,
        dataset=dataset,
        eval_offline_fns=[eval_loss, eval_loss_q],
