...
def build_d3rl_policy() -> tuple[DeterministicPolicy, torch.optim.Optimizer]:
...
def d3rl_predict_best_action(
    policy: DeterministicPolicy, obs_np: np.ndarray
) -> np.ndarray:
...
def _pairs(our_agent: BCAgentContinuousDeterministic, d3_policy: DeterministicPolicy):
    return [
...
def main() -> None:
    ...
    for i in range(1, N_TEST_BATCHES + 1):
        ...
        if not np.array_equal(d3_act, our_act):
            ...
            raise SystemExit(
                f"FAIL: action mismatch (d3rl vs bc_agent_continuous) at batch={i}, max_abs_diff={max_diff:.12e}"
            )
    ...
    for i in range(1, N_TEST_BATCHES + 1):
        ...
        our_pred = our_agent.policy(obs_t)
        our_loss = our_agent._loss(our_pred, act_t)
        d3_loss = compute_deterministic_imitation_loss(
            policy=d3_policy,
            x=obs_t,
            action=act_t,
        )
        ...
        if loss_diff != 0.0:
            raise SystemExit(
                f"FAIL: loss mismatch at batch={i}, loss={loss_diff:.12e}"
            )
