...
from ice_offline.agent.cql_agent_continuous_max_q_backup import CQLAgentContinuousMaxQBackup
...
# 1) Flow Function
# ====================
def build_our_agent() -> CQLAgentContinuousMaxQBackup:
    return CQLAgentContinuousMaxQBackup(obs_size=OBS_DIM, act_size=ACT_DIM)

def build_d3rl():
    config = d3rlpy.algos.CQLConfig(soft_q_backup=False, max_q_backup=True)
...
def copy_d3rl_weights_to_our(algo, our: CQLAgentContinuousMaxQBackup) -> None:
    with torch.no_grad():
...
def _our_losses(
    our: CQLAgentContinuousMaxQBackup,
    obs_t: torch.Tensor,
    act_t: torch.Tensor,
    rew_t: torch.Tensor,
    ...
    actor_action, actor_log_prob = our._sample_a(obs_t)
    temp = -(our.log_alpha_sac.exp() * (actor_log_prob - our.act_size)).mean()
    actor = our._loss_actor(obs_t, actor_action, actor_log_prob)
    return torch.stack([critic, actor, temp])
...
def _d3rl_losses(d3rl, batch: TorchMiniBatch, obs_t: torch.Tensor) -> torch.Tensor:
    ...
    entropy = get_parameter(d3rl.impl.modules.log_temp).exp() * log_prob
    q_t = d3rl.impl._q_func_forwarder.compute_expected_q(obs_t, sampled_action, "min")
    actor = (entropy - q_t).mean()
    temp = -(get_parameter(d3rl.impl.modules.log_temp).exp() * (log_prob - ACT_DIM)).mean()
    return torch.stack([critic, actor, temp])

def _all_pairs(our: CQLAgentContinuousMaxQBackup, algo):
