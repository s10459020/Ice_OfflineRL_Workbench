...
def plot(task_id: str, rows: EvalRows) -> Path:
    ...
    output_path = plot_path("experience_a", dataset_id, agent_id)
...
def eval(task_id: str, eval_path: Path) -> EvalRows:
    ...
    returns_output_path, steps_output_path = write_eval_rows(EXPERIMENT, task_id, returns_rows, steps_rows)
