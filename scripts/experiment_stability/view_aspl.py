...
def save_tables_group(group: str, dataset_id_list: list[str], agent_id_list: list[str]) -> tuple[Path, ...]:
    ...
    step_values = [
        [_agent_steps(dataset_id, agent_id) for agent_id in agent_id_list]
        for dataset_id in dataset_ids
    ]
    upper_step_values = [_dataset_steps(upper_id) for upper_id in upper_ids]
    ...
    return (
        ...
        table_mean_steps(
            ...
            step_values,
            upper_step_values,
            table_path(group, "mean_steps.csv"),
        ),
        table_diff_steps(
            dataset_ids,
            agent_id_list,
            ...
            step_values,
            upper_step_values,
            table_path(group, "diff_steps.csv"),
        ),
    )
