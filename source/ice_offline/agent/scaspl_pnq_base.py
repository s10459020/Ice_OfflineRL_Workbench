...
class ScasplPNQBaseAgent(ScasplPNAgent):
    ...
    def update_critic(self, batch: Batch) -> MetricValues:
        ...
        loss_td_clean, metrics_td_clean = self.loss_td_clean(batch, target)
        self.critic_optimizer.zero_grad()
        loss_td_clean.backward()
        ...
        metrics_critic_punish = {
            ...
            "grad_critic_punish": self._grad_norm(loss_critic_punish, self.critic_punish.param_critic()),
        }
        self.critic_punish_optimizer.zero_grad()
        loss_critic_punish.backward()
        self.critic_punish_optimizer.step()

        return metrics_td_clean | metrics_td_punish | metrics_aspl_punish | metrics_critic_punish | {
            "q_punish_ema": self._value(self.critic_punish.q_avg.detach()),
        }
    ...
    def loss_td_clean(self, batch: Batch, target: torch.Tensor) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        ...
        q_values = self.critic.q_all(observation, action)
        q_min = torch.cat(q_values, dim=1).min(dim=1, keepdim=True).values
        loss = sum(F.mse_loss(q_value, target) for q_value in q_values)
        return loss, {
            "loss_td_clean": self._value(loss.detach()),
            "grad_td_clean": self._grad_norm(loss, self.critic.param_critic()),
            "loss_critic_clean": self._value(loss.detach()),
            "grad_critic_clean": self._grad_norm(loss, self.critic.param_critic()),
            "target_clean": self._value(target.mean().detach()),
            "q_clean_data": self._value(q_min.mean().detach()),
        }

    def loss_td_punish(self, batch: Batch, target: torch.Tensor) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        observation, action, _, _, _ = batch
        q_values = self.critic_punish.q_all(observation, action)
        q_min = torch.cat(q_values, dim=1).min(dim=1, keepdim=True).values
        loss = sum(F.mse_loss(q_value, target) for q_value in q_values)
        return loss, {
            "loss_td_punish": self._value(loss.detach()),
            "grad_td_punish": self._grad_norm(loss, self.critic_punish.param_critic()),
            "target_punish": self._value(target.mean().detach()),
    ...
    def loss_punish_pnq(self, batch: Batch) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        ...
        return loss, {
            "loss_aspl_punish": self._value(loss.detach()),
