...
class ActorCriticAgent:
    ...
    def _delta_q(self, obs_vector: np.ndarray, action: int, reward: float, next_obs_vector: np.ndarray, done: bool) -> np.float32:
        # Q(s, a) = E[r + grama * max_a{Q(s', a)}]
        #
        # delta_q = Q_target - Q
        #         = Q_target(s, a)              - Q(s, a)
        #         = r + gamma * max_a{Q(s', a)} - Q(s, a)
        q_t = self._Q(obs_vector, action)
        next_q = np.float32(0.0) if done else np.float32(np.max(self._Q_all(next_obs_vector)))
        return np.float32(reward + self.gamma * next_q - q_t)
