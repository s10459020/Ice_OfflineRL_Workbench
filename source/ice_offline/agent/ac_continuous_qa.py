...
class ActorCriticAgent:
    ...
    def update(self, o: np.ndarray, a: np.ndarray, r: float, o_: np.ndarray, done: bool) -> None:
        ...
        grad_qW, grad_qb = self._nabla_L(obs_vector, act_vector, float(r), next_obs_vector, bool(done))
    ...
    def _V(self, obs_vector: np.ndarray) -> np.float32:
        # V(s) = E_pi[Q(s, a)]
        #      = Q(s, E_pi[a]) for linear Q in a
        mean = self._mean(obs_vector)
        return self._Q(obs_vector, mean)
    ...
    def _nabla_L(self, obs_vector: np.ndarray, act_vector: np.ndarray, reward: float, next_obs_vector: np.ndarray, done: bool) -> tuple[np.ndarray, np.float32]:
        # Loss Q = MSE(delta_q)
