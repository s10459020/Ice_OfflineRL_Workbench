...
class _Critic(torch.nn.Module):
    ...
    def eval_target_q_n(self, o: torch.Tensor, a_sample: torch.Tensor) -> torch.Tensor:
...
class CQLMaxQAgent(TorchAgent):
    ...
    def td_target(self, on: torch.Tensor, r: torch.Tensor, d: torch.Tensor) -> torch.Tensor:
        ...
        #
        # max_q_backup(max Q): True
        # soft_q_backup(SAC form): False 
        with torch.no_grad():
            batch = on.shape[0]
            an, _ = self.actor.sample_n(on, self.critic.n_action_samples)
            qn = self.critic.eval_target_q_n(on, an).view(2, batch, self.critic.n_action_samples)
            qn = qn.min(dim=0).values.max(dim=1, keepdim=True).values
