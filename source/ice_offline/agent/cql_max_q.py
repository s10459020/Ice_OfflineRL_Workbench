...
class CQLAgentMaxQ(TorchAgent):
    ...
    def target(self, on: torch.Tensor, r: torch.Tensor, d: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            batch = on.shape[0]
            an, _ = self.policy.sample_n(on, self.critic.n_action_samples)
            qn = self.critic.eval_tq_n(on, an).view(2, batch, self.critic.n_action_samples)
