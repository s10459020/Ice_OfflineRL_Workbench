...
class DistributionWrapper(gym.Wrapper):
    ...
    def __init__(self, env: gym.Env) -> None:
        if not isinstance(env, RenderOverlayWrapper):
            raise TypeError("DistributionWrapper requires env to be RenderOverlayWrapper.")
        super().__init__(env)
        ...
        self.env.register_overlay(
            name="distribution",
            fn=self._overlay_distribution,
            layer=int(RenderLayer.DISTRIBUTION),
            z=0,
            enabled=True,
        )
    ...
    # ---------------------------------------------------------------------
    # Overlay callback (called by RenderOverlayWrapper)
    # ---------------------------------------------------------------------
    def _overlay_distribution(self, tile_img: np.ndarray, ctx: dict[str, Any]) -> None:
        ...
        if self._render_style == "rect12":
            self._draw_rect12_overlay(tile_img, cell_values, self._frame_vmin, self._frame_vmax)
            return
        tile_float = self._draw_ring12_overlay(
            tile_img.astype(np.float32),
            cell_values,
            self._get_sector_map(tile_size),
            tile_size,
            self._frame_vmin,
            self._frame_vmax,
        )
        tile_img[:, :, :] = np.clip(tile_float, 0.0, 255.0).astype(np.uint8)
    ...
    def _draw_rect12_overlay(
        ...
        for d_idx in MiniGridDirection:
            for slot_idx, a_real in enumerate(action_order):
                ...
                tile_img_u8[masks[d_idx, slot_idx, level]] = color
